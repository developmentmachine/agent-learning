...
在构建企业级 Agent 时，我们会面临三个传统软件开发中少见的挑战：

1.  **非确定性 (Non-determinism)**：即便 Prompt 没变，大模型在不同时刻的输出也可能略有差异。这意味着“跑通一次”不代表“功能稳定”，必须通过多次试验（如 `pass@k`）来衡量成功率。
2.  **黑盒化与行为漂移 (Black-box Drift)**：底层模型升级、Prompt 微调或 RAG 知识库的变化，都可能导致 Agent 行为发生不可感知的偏离（例如突然忘记调用某个工具），这被称为“静默退化”。
3.  **错误级联放大 (Cascading Errors)**：Agent 往往涉及多步推理。第一步工具调用（Observation）中的微小偏差，会沿着执行链路逐级放大，最终导致结论完全偏离目标。

因此，我们需要一套能够捕获**执行轨迹 (Trace)** 并在多个维度进行**自动化评分**的体系。
